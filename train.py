...
def parse_args():
    ...
    parser.add_argument('--pool_method', type=str, default='add', help='type of global pooling layer at the end of the encoder. Choose between add, max, mean')
...
def train(train_loader, validation_dataset, model, optimizer, scheduler, 
          criterion_verb, criterion_rels,
          config, batch_size,
          num_epochs, device,
          proj_dim, hidden_dim, output_dim, 
          pool_method, wandb_log, graph_type, lr_start):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            optimizer.zero_grad()
            out_verb, out_rels = model(batch)  # model(batch.x, batch.edge_index)  
            
            ####
            l1 = criterion_verb(out_verb, batch.y[0])
            l2 = criterion_rels(out_rels, batch.y[1])        
        ...
        average_loss = total_loss / (count*batch_size)
        ...
        #         }
        #         if wandb_lo g: wandb.log(recalls_dict)
    ...
    torch.save(model.state_dict(), f'trained_models/easg_diffusion{num_epochs}_{lr_start}_{graph_type}_{pool_method}_pd={proj_dim}_hd={hidden_dim}_outd={output_dim}.pth')
    print('Model saved!')
    recalls = evaluation(validation_dataset, model, device)
    recall_predcls_with, recall_predcls_no, recall_sgcls_with, recall_sgcls_no, recall_easgcls_with, recall_easgcls_no = recalls
...
def main():
    ...
    autoencoder = EASGAutoEncoder(obj_dim, verb_dim, 
                                ...
                                args.dropout_prob, args.graph_type)
    ...
    criterion_rels = nn.BCEWithLogitsLoss()
    ...
    config = set_wandb_config(args.num_epochs, args.hidden_proj_dim, args.proj_dim, 
                        ...
                        args.pool_method, args.dropout_prob)

    # TRAIN THE MODEL
    train(train_loader, validation_dataset, autoencoder, optimizer, scheduler, 
          criterion_verb, criterion_rels, 
          config, args.batch_size,
          args.num_epochs, device, 
          args.proj_dim, args.hidden_dim, args.output_dim, 
          args.pool_method, args.wandb, args.graph_type, args.lr_start)    
